...
df_X = df.iloc[0000:][cols]
df_y = df.iloc[0000:]["Close"]
...
for d in dates[:-1]:
    print(d)
    mask = (df_X.index.date == d) & (df_X.index.hour >= 2)
    X = df_X[mask].iloc[:18*12].to_numpy()
    Y = df_y[mask].iloc[horizon:18*12+horizon].to_numpy()
...
print("(pca)states_tr.shape: ", states_tr.shape)
print("(pca)states_te.shape: ", states_te.shape)

print("(pca)states_tr: ", states_tr)
print("(pca)states_te: ", states_te)
...
# plt.show()
plt.savefig("predictions_ridge3.png")
...
plt.grid()
plt.legend()
plt.title("Predicted stock index value")
# plt.show()
plt.savefig("predictions_gbrt3.png")
