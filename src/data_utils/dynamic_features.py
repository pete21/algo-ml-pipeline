...
def dynamic_features(df, parameters, scaler, col_close="close", col_high="high", col_low="low") -> pd.DataFrame:
    ...
    df['kama_trend_slow'] = df['kama_trend_slow']*50/scaler
    df['kama_trend_fast'] = df['kama_trend_fast']*50/scaler
    df['kama_diff'] = df['kama_diff']/scaler

    df['kama_trend_slow_diff'] = df['kama_trend_slow'].diff()*100
    df['kama_trend_fast_diff'] = df['kama_trend_fast'].diff()*50
    df['kama_trend_slow_diff2'] = df['kama_trend_slow_diff'].diff()*10
    df['kama_trend_fast_diff2'] = df['kama_trend_fast_diff'].diff()*10

    df['ema_ha_wickstrength'] = df['ema_ha_wickstrength']/scaler

    df.loc[:,'atr'] = talib.ATR(df[col_high], df[col_low], df[col_close], timeperiod=parameters['atr_period'])/df['ha_close']*100/scaler
    ...
    df.loc[:,'cci_slope'] = talib.LINEARREG_SLOPE(df['cci'], parameters['cci_slope_period'])
    ...
    # df.loc[:,'macdhist'] = df['macdhist']/scaler
    # df.loc[:,'macd_slope'] = talib.LINEARREG_SLOPE(df['macd'], parameters['macd_slope_period'])
    df.loc[:,'macdhist_slope'] = talib.LINEARREG_SLOPE(df['macdhist'], parameters['macd_slope_period'])/df['ha_close']*1000/scaler

    df.loc[:,'rsi'] = talib.RSI(df[col_close], timeperiod=parameters['rsi_period'])/100
    df.loc[:,'rsi_ha'] = talib.RSI(df['ha_close'], timeperiod=parameters['rsi_period'])/100
    df.loc[:,'rsi_slope'] = talib.LINEARREG_SLOPE(df['rsi'], parameters['rsi_slope_period'])*100
    ...
    df.loc[:,'stochk'] = df['stochk']/100
    df.loc[:,'stochd'] = df['stochd']/100

    df.loc[:,'stochk_slope'] = talib.LINEARREG_SLOPE(df['stochk'], parameters['stochk_slope_period'])*100
    ...
    df.loc[:,'stochrsik'] = df['stochrsik']/100
    df.loc[:,'stochrsid'] = df['stochrsid']/100

    df.loc[:,'stochrsik_slope'] = talib.LINEARREG_SLOPE(df['stochrsik'], parameters['stochrsik_slope_period'])*100
    ...
    df.loc[:,'ppo_slope'] = talib.LINEARREG_SLOPE(df['ppo'], parameters['ppo_slope_period'])*100

    df.loc[:,'willr'] = talib.WILLR(df.High, df.Low, df.Close, timeperiod=parameters['willr_timeperiod'])/100+1

    # Compute Hilbert Transform Dominant Cycle
    #df['Hilbert_Dominant_Cycle'] = hilbert_dominant_cycle(df[col_close])

    df.loc[:,'adx'] = talib.ADX(df[col_high],df[col_low],df[col_close],timeperiod=parameters['adx_timeperiod'])/100
    df.loc[:,'di_plus'] = talib.PLUS_DI(df[col_high],df[col_low],df[col_close],timeperiod=parameters['di_timeperiod'])/50
    df.loc[:,'di_minus'] = talib.MINUS_DI(df[col_high],df[col_low],df[col_close],timeperiod=parameters['di_timeperiod'])/50
