...
from src.lib.wavelet import wavelet_denoising2
...
def load_data(params: dict, data_path: str) -> dict:
    ...
    try:
        ...
        for i in params['data_ingestion']['indexes']:
            file = params['data_ingestion']['file_name'].format(timeframe=params['data_ingestion']['timeframes'][i])     #DAX40_USDT_USDT-10s-futures.parquet
            print(file)
            # Import the data
            dax_data[i] = pd.read_parquet(data_path + '/src/' + file)
...
def preprocess_data(dax_data: dict, params: dict) -> dict:
    ...
    try:
        ...
        dax_data[params['data_ingestion']['index_base']]["date_merge"] = dax_data[params['data_ingestion']['index_base']].index
        dax_data[params['data_ingestion']['index_base']] = dax_data[params['data_ingestion']['index_base']].iloc[:,0:8]
...
def save_data(data: dict, params: dict, data_path: str) -> None:
    ...
    try:
        ...
        data[params['data_ingestion']['index_base']].to_csv(f'{raw_data_path}/data_ohlc_features_{params['data_ingestion']['timeframes'][params['data_ingestion']['index_base']]}.csv')

        for i in params['data_ingestion']['indexes_higher']:
            print(f'Timeframe: {params['data_ingestion']['timeframes'][i]}')
            data[i].to_csv(f'{raw_data_path}/data_ohlc_features_{params['data_ingestion']['timeframes'][i]}.csv')
...
def main():
    try:
        ...
        dax_data = load_data(params, data_path)
        
        # Preprocess the data
        data = preprocess_data(dax_data, params)

        unique_dates, unique_weekdates, mondays_indexes = get_dates(data, params['data_ingestion']['index_base'])
        
        # Save the data
        save_data(data, params, data_path)
