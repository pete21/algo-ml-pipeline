...
from src.data_utils.features import build_target, build_target_2
...
hist, bins = np.histogram(ml_data[params['index_base']]["labeling_multi"])
print("labeling_multi distribution original target: ", hist, bins)

ml_data[params['index_base']] = build_target_2(ml_data[params['index_base']], labeling_col="labeling_dual_ema", threshold=0.45)

hist, bins = np.histogram(ml_data[params['index_base']]["labeling_multi"])
print("labeling_multi distribution new target: ", hist, bins)
...
hist, bins = np.histogram(ml_data[params['index_base']]["labeling_multi"])
print("labeling_multi distribution trade hours: ", hist, bins)
...
num_windows_per_day = 6

window_step_minute = 15

scores = {}

for min_window_end_minute in range(520, 525, 5):

    max_window_end_minute = min_window_end_minute+num_windows_per_day*window_step_minute

    print(len(dates))

    # If exists, load Xy_sum from a pickle file
    if os.path.exists('Xy_sum.pkl'):
        with open('Xy_sum.pkl', 'rb') as f:
            Xy_sum = pickle.load(f)
    else:
        Xy_sum = []
        ...
        for d in dates:
            ...
            if X_columntransform.index.size < (min_window_end_minute-60)/params['timeframe_minutes'][params['index_base']]:
        ...
        # Save Xy_sum to a pickle file
        with open('Xy_sum.pkl', 'wb') as f:
            pickle.dump(Xy_sum, f)
    ...
    balanced_acc, f1, syntetic_score = compute_test_scores(pred_class, yte_concat)
    print(f"Balanced Accuracy = {balanced_acc:.3f}, F1 = {f1:.3f}")
    print(f"Syntetic score = {syntetic_score:.3f}")
    ...
    scores[min_window_end_minute] = {
        'accuracy': balanced_acc,
        'f1': f1,
        'syntetic_score': syntetic_score,
        'diff': diff,
